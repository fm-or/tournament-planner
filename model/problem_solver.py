...
class ProblemSolver:
    ...
    def __init__(self,
        ...
        self._block_count = ceil(self._game_count/field_count)
    ...
    def _solve(self, max_consecutive_games: int, max_consecutive_pauses: int, max_field_deviation: int, prioritized_solver_str: Optional[str], output: bool) -> TournamentPlan:
        # variable definition
        x = dict() # matchups
        z = dict() # referees
                ...
                for g, group in enumerate(self.groups):
                    for t1 in range(group.size):
                        for t2 in range(group.size):
                            if t1 != t2:
                                # does team t1 play vs team t2 from group g in block b on field f on side 0
            ...
            for f in range(self.field_count):
                for g, group in enumerate(self.groups):
                    for t in range(group.size):
                        # is team t the referee in block b on field f?
            ...
            for t in range(group.size):
                ...
                for t2 in range(group.size):
                    if t != t2:
                        ...
                        prob += lpSum(x[b, f, g, t, t2] + x[b, f, g, t2, t] for b in range(self.block_count) for f in range(self.field_count)) == 1
                        for b in range(self.block_count):
                            for f in range(self.field_count):
                                # order does matter
                                prob += x[b, f, g, t, t2] + x[b, f, g, t2, t] <= 1
                # optional: each team plays groups[g]-1 games
                prob += lpSum(x[b, f, g, t, t2] + x[b, f, g, t2, t] for b in range(self.block_count) for f in range(self.field_count) for t2 in range(group.size) if t != t2) == group.size - 1
                # each team plays at least floor((max(groups)-1)/fields) on each field: todo
                for f in range(self.field_count):
                    prob += lpSum(x[b, f, g, t, t2] + x[b, f, g, t2, t] for b in range(self.block_count) for t2 in range(group.size) if t != t2) >= floor((self.max_group_size-1)/self.field_count) - max_field_deviation
                # each team plays at most once per block
                for b in range(self.block_count):
                    prob += lpSum(x[b, f, g, t, t2] + x[b, f, g, t2, t] for f in range(self.field_count) for t2 in range(group.size) if t != t2) <= 1
                # each team plays half the time on each side (approximately)
                prob += lpSum(x[b, f, g, t, t2] - x[b, f, g, t2, t] for b in range(self.block_count) for f in range(self.field_count) for t2 in range(group.size) if t != t2) <= max_side_deviation
                prob += lpSum(x[b, f, g, t2, t] - x[b, f, g, t, t2] for b in range(self.block_count) for f in range(self.field_count) for t2 in range(group.size) if t != t2) <= max_side_deviation
                # maximum number of consecutive games
                for b_first in range(self.block_count - max_consecutive_games):
                    prob += lpSum(x[b, f, g, t, t2] + x[b, f, g, t2, t] for b in range(b_first, b_first + max_consecutive_games + 1) for f in range(self.field_count) for t2 in range(group.size) if t != t2) <= max_consecutive_games
                # maximum number of consecutive pauses
                for b_first in range(self.block_count - max_consecutive_pauses):
                    prob += lpSum(x[b, f, g, t, t2] + x[b, f, g, t2, t] for b in range(b_first, b_first + max_consecutive_pauses + 1) for f in range(self.field_count) for t2 in range(group.size) if t != t2) >= 1
                # maximum number of referee games
                prob += lpSum(z[b, f, g, t] for b in range(self.block_count) for f in range(self.field_count)) <= max_referee_games
                # a team can not play and be a referee in the same block
                for b in range(self.block_count):
                    prob += lpSum(x[b, f, g, t, t2] + x[b, f, g, t2, t] for f in range(self.field_count) for t2 in range(group.size) if t != t2) <= 1 - lpSum(z[b, f, g, t] for f in range(self.field_count))

        for b in range(self.block_count):
            if b <= self.block_count-2:
                # every block but the last one is full
                prob += lpSum(x[b, f, g, t, t2] for f in range(self.field_count) for g, group in enumerate(self.groups) for t in range(group.size) for t2 in range(group.size) if t != t2) == self.field_count
                for f in range(self.field_count):
                    # at most one game per field
        ...
        # solve
        if output:
            print(f"Solving problem configuration ({max_consecutive_games},{max_consecutive_pauses},{max_field_deviation}).  ", end='\r')
        # check for solvers
    ...
    def solve(self,
              ...
              max_field_deviation_fixed: Optional[int] = None,
        ...
        max_field_deviation_intervall = (0, floor((self.max_group_size-1)/self.field_count)) # maximum deviation from the average field attendance {0, ..., floor((teams-1)/fields)}
        ...
        if max_consecutive_games_fixed is None:
            ...
            max_field_deviation = max_field_deviation_intervall[1] if max_field_deviation_fixed is None else max_field_deviation_fixed
            ...
            max_field_deviation = max_field_deviation_intervall[1] if max_field_deviation_fixed is None else max_field_deviation_fixed
        else:# elif max_field_deviation_fixed is None:
            max_consecutive_games = max_consecutive_games_fixed
            max_consecutive_pauses = max_consecutive_pauses_fixed
            max_field_deviation = max_field_deviation_intervall[0]

        plan = None
        while max_consecutive_games_fixed is None or max_consecutive_pauses_fixed is None or max_field_deviation_fixed is None:
            # solve problem
            plan = self._solve(max_consecutive_games, max_consecutive_pauses, max_field_deviation, prioritized_solver_str, output)
            ...
            if plan is None:
                ...
                elif max_field_deviation_fixed is None:
                    if max_field_deviation > max_field_deviation_intervall[1]:
                        break
                    max_field_deviation += 1
            else:
                if max_consecutive_games_fixed is None:
                    ...
                    max_field_deviation = max_field_deviation_intervall[1] if max_consecutive_pauses_fixed is None else max_field_deviation_intervall[0] 
                elif max_consecutive_pauses_fixed is None:
                    max_consecutive_pauses_fixed = max_consecutive_pauses
                    max_field_deviation = max_field_deviation_intervall[0] if max_field_deviation_fixed is None else max_field_deviation_fixed
                elif max_field_deviation_fixed is None:
                    max_field_deviation_fixed = max_field_deviation
        if plan is None:
